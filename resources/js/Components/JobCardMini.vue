...
<template>
    <div class="flex md:flex-row flex-col justify-between bg-gray-500/10 p-4 m-2 rounded-md border  hover:bg-gray-700/20 mt-3 hover:border-blue-800 group" :class="active ? 'border-blue-600': 'border-gray-400'">
        ...
        <div class="basis-2/3 pb-2 md:pb-0">
            <a href="#"><h3 class="font-bold pb-1 underline group-hover:text-blue-800 transition-colors duration-900">{{ job.job_title }}</h3></a>
            <div class="text-sm pr-2 md:pr-0">
                <div class="text-gray-800 ">{{ job.company_name }}</div>
                <div class="text-xs">Location: {{  job.location }}</div>
                <p class="text-xs">{{ job.employment_type }} - {{  job.salary_min }}</p>
            </div>
        </div>
    </div>
</template>
